 ...
 */
type FormValueOf<T extends IPayment | NewPayment> = Omit<T, 'dateTime'> & {
...
type PaymentFormDefaults = Pick<NewPayment, 'id' | 'dateTime'>;
...
type PaymentFormGroupContent = {
  ...
  dateTime: FormControl<PaymentFormRawValue['dateTime']>;
  status: FormControl<PaymentFormRawValue['status']>;
  player: FormControl<PaymentFormRawValue['player']>;
};
...
export class PaymentFormService {
  createPaymentFormGroup(payment: PaymentFormGroupInput = { id: null }): PaymentFormGroup {
    ...
    return new FormGroup<PaymentFormGroupContent>({
      ...
      dateTime: new FormControl(paymentRawValue.dateTime),
      status: new FormControl(paymentRawValue.status),
      player: new FormControl(paymentRawValue.player),
    });
  }
  ...
  private convertPaymentRawValueToPayment(rawPayment: PaymentFormRawValue | NewPaymentFormRawValue): IPayment | NewPayment {
    return {
      ...rawPayment,
      dateTime: dayjs(rawPayment.dateTime, DATE_TIME_FORMAT),
    };
  }
  ...
  private convertPaymentToPaymentRawValue(
    ...
    return {
      ...payment,
      dateTime: payment.dateTime ? payment.dateTime.format(DATE_TIME_FORMAT) : undefined,
    };
  }
}
